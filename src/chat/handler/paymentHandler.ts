...
if (!email) throw new Error('EMAIL env variable not set')

const paystackPaymentHandler = async ({ session }: { session: any }): Promise<ChatResponse> => {
...
const verifyPaystackPaymentHandler = async ({ session, reference }: { session: any, reference: string}): Promise<ChatResponse> => {
