...
const handleOrdering = async ({ input, session }) => {
    ...
    if (input === '0') {
        ...
        const totalPrice = (await Promise.all(session.temporaryOrder.map(async (item) => {
    ...
    const result = await selectMenuItem(input, menu, session, ChatState.ORDERING, session.temporaryOrder.length === 0 // first selection
    );
    return result;
};
